...
// Include country code, but DO NOT include +, spaces, brackets or dashes.
//
...
const serviceMap: Record<ServiceId, ServiceMeta> = {
  "Flight Bookings": {
    ...
    fields: [
      ...
      {
        id: "returnDate",
        label: "Return Date (blank = one-way)",
        type: "date",
      },
      {
        id: "adults",
              ...
              filtered.map((country) => (
                <button
                  ...
                  className={`flex w-full items-center gap-3 px-4 py-2.5 text-left text-sm transition-colors hover:bg-gray-50 ${
                    value === country.name
                      ? "bg-blue-50 font-semibold text-[#0071c2]"
                      : "text-gray-700"
                  }`}
                >
                  ...
                  {value === country.name && (
                    <span className="ml-auto text-[#0071c2]">
                      ✓
                    </span>
                  )}
                </button>
...
function buildWhatsAppMessage(
  ...
  fields.forEach((field) => {
    if (
      clientFieldIds.includes(field.id) &&
      getValue(field.id)
    ) {
  ...
  fields.forEach((field) => {
    ...
    const rawValue = getValue(field.id)
  ...
  lines.push(
    "🙏 Thank you! I would like to proceed with this request."
  )
...
export function BookingSearch({
  ...
  const [service, setService] =
    useState<ServiceId>(serviceName)
  ...
  const handleSubmit = async () => {
    ...
    try {
      // ─────────────────────────────────────────────────────────────────────
      // 1. Send request to your API/email system
      // ─────────────────────────────────────────────────────────────────────
      ...
      //
      // encodeURIComponent is important because the message contains:
  ...
  return (
    <section
      id="booking"
      className="relative z-30 bg-secondary px-3 pb-14 pt-24 sm:px-4 md:pt-28"
    >
            ...
            {services.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`group relative flex items-center gap-2 whitespace-nowrap rounded-full border px-4 py-2.5 text-sm font-semibold transition-all duration-200 ${
                    active
          ...
          {status === "success" ? (
            <div className="flex flex-col items-center justify-center gap-4 px-8 py-16 text-center">
              ...
              <p className="max-w-md text-gray-500">
                ...
                WhatsApp is opening with your
                booking summary ready to send.
                💬
              </p>
            ...
            <div>
              ...
              <div className="flex flex-col items-center justify-between gap-4 rounded-b-xl border-t bg-gray-50/60 px-6 py-4 sm:flex-row">
                ...
                <div className="text-sm text-gray-500">
                  ...
                  <p className="mt-0.5 text-xs text-gray-400">
                    After submitting, WhatsApp
                    will open with your booking
